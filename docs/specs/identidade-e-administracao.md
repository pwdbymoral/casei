...
Preferências do espaço — moeda, fuso IANA, nome e margem de segurança — são editáveis pelo owner, com prévia explícita das consequências. A leitura e a atualização do espaço são escopadas pelo membership server-side e exigem `If-Match`; conflitos retornam `412` com a versão atual. Moeda não pode mudar quando já existe movimento publicado, qualquer transação/compromisso `planned`/`partially_settled`, qualquer cartão de crédito ou qualquer meta/movimento de reserva cadastrado; registrar um cartão ou uma meta torna a moeda imutável mesmo sem lançamentos. Leitura, criação, liquidação e alocação de reserva serializam pelo lock do espaço para não publicar a moeda antiga durante uma alteração. Antes disso, a alteração é auditada e validada no servidor. Auditoria de perfil/preferências guarda somente `before_redacted`/`after_redacted` com campos allowlisted (flags, locale, moeda e fuso; nome e margem como `[redacted]`), nunca senha, token, e-mail completo ou conteúdo financeiro.
...
- [x] Perfil e preferências permitem editar nome/locale/ocultação de valores, iniciar reverificação de e-mail e senha e, para owner, editar nome/fuso/margem de segurança do espaço com prévia, `If-Match` e bloqueio de mudança de moeda após movimentos, compromissos, cadastro de cartão ou meta/reserva.
