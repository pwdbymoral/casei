...
- [x] **DATA-004 Aplicação:** `ImportApplication` transforma linhas do preflight em comandos de domínio por meio de `ImportCommandPort`, com job versionado `data.import:1`, lotes transacionais, revalidação de ator/capacidade e expiração antes de cada lote, atomicidade por linha em `valid_only`, rollback do lote em `all_or_nothing`, retry por chave estável, cancelamento cooperativo e reversão auditável por linha. `PostgresImportStore` persiste cursor, contagens e resultados e `createImportWorker` registra o handler durável; storage binário e adapters de domínio permanecem injetados por DATA-001/FIN/STOCK.
