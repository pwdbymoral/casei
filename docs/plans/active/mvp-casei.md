...
- [x] **DATA-001 ADR e adapters de arquivo:** `@casei/storage` define `ObjectStoragePort` e `FileScanPort`, implementa adapter S3-compatible com upload/download bounded em stream, SHA-256, expiração lógica até 24 horas, SSE-S3, chaves opacas, MIME/magic validation e scanner de malware injetável; configuração `CASEI_OBJECT_STORAGE_*` é explícita e não publica credenciais ou URL bearer.
