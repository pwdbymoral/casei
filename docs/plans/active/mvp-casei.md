...
- [x] **PLAT-004 Infra de comandos:** `@casei/database` fornece unit-of-work com contexto RLS, idempotência por hash canônico e chave escopada, outbox→job transacional e `PostgresJobWorker` com `FOR UPDATE SKIP LOCKED`, lease/compare-and-set, backoff, retry/dead-letter e handlers em lotes transacionais. `job` preserva ator, espaço, capacidade e correlation ID; cada lote e transição bloqueia/revalida membership e capacidade. `packages/database/test/command-infra.test.ts` cobre replay, conflito, concorrência, outbox e execução autorizada; a integração PostgreSQL do CI cobre locks/RLS e cenários descartáveis.
