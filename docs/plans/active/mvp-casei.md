...
- [x] **AUTH-006 Perfil e preferências:** API e PWA para editar nome/locale (`pt-BR` no MVP)/ocultação de valores e iniciar os fluxos nativos Better Auth de senha/e-mail com reverificação; owner edita nome, fuso IANA e margem de segurança do espaço com `If-Match`, prévia e bloqueio server-side de mudança de moeda após o primeiro movimento publicado. Migration `0004_profile_preferences` tem companion down; contratos, RLS/autorização cruzada, auditoria redigida, estados loading/error/offline/conflito e testes focados estão cobertos em `identity-routes`, `identity-service.integration`, `settings.test` e `database/test/schema` (a integração PostgreSQL roda quando `DATABASE_URL_TEST` está disponível).
