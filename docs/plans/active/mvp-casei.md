...
- [x] **AUTH-002 Onboarding:** criação idempotente de espaço/owner, moeda/fuso/saldo inicial opcional e retomada após falha. `IdentityService.createOnboarding` usa `executeIdempotent` e a migration `0003_identity_workspaces` persiste o contexto do onboarding.
- [x] **AUTH-003 Memberships:** convite com token armazenado somente como hash, reenvio que revoga o token anterior, expiração, aceite com e-mail correspondente, remoção e transferência de propriedade.
- [x] **AUTH-004 Autorização:** matriz owner/member/viewer, escopo server-side por `workspaceId`, membership lock em mutações, política RLS para memberships próprios e middleware de sessão/escopo; rotas não autorizadas não confiam no ID fornecido pelo cliente.
- [x] **AUTH-005 Desativação do espaço:** confirmação do owner com autenticação recente, estado `deletion_pending`, entitlement de recuperação por 30 dias sem revogar identidade/outros espaços, cancelamento de jobs/outbox, purge idempotente por workspace no dia 30 e tombstone pseudonimizado com retenção de auditoria por 365 dias.
