...
Callbacks `after` de Better Auth não são a fronteira de atomicidade: a partir da linha 1.5 eles executam depois do commit, e o fluxo de verificação pode entregar um JWT assinado diretamente ao callback sem persistir uma linha de token no adapter. Portanto, não dependemos de uma linha `verification` para garantir entrega. Cada operação de cadastro/verificação/reset cria antes um registro `auth_email_intent` idempotente, com finalidade, ator opcional até o usuário existir, e-mail normalizado protegido, correlation ID, callback URL allowlisted, estado e expiração. O callback `sendVerificationEmail`/reset recebe a URL/token e transforma a intent em payload criptografado de `auth_email_outbox`; token, URL e conteúdo nunca aparecem em logs. Se o callback falhar depois do commit, a intent pendente aciona recovery job que solicita um novo token pelo endpoint/API da versão pinada e reprocessa a outbox; o usuário vê estado pendente e pode reenviar. Convites e outras mensagens iniciadas pelo Casei gravam a outbox na própria transação do comando. No fluxo de convite do MVP, `workspace.invitation_created` é uma intenção metadata-only (`invitationId`, e-mail e papel); o token não é persistido no payload nem registrado em logs, e o owner recebe o link de uso único na resposta/UI para copiar por um canal seguro. A entrega assíncrona de e-mail de convite consumirá essa intenção em uma fatia posterior com payload protegido, sem alterar o comando ou expor o token. O worker entrega com idempotência por `(message_kind, source_id)` e reintenta sem criar mensagem duplicada. AUTH-001 é bloqueada até um spike contra `better-auth@1.6.22` provar cadastro, reenvio, reset, falha da outbox, recovery, preservação da callback URL e expiração; qualquer divergência exige nova ADR. Respostas públicas não revelam existência de conta.

O Better Auth `runInBackgroundOrAwait` registra e absorve rejeições do callback. Por isso a aplicação tenta novamente a persistência com limite curto e, se a indisponibilidade persistir, grava a mensagem em um spool local append-only criptografado e persistente (`CASEI_AUTH_EMAIL_RECOVERY_SPOOL`). O worker drena esse spool antes de reivindicar a outbox após cada restart; o adapter padrão de produção não depende apenas de log ou memória e o spool nunca contém token/URL em claro. A resposta pública não é usada como confirmação de entrega. A inserção da intent e da outbox, e as transições `queued`/`sent`/`failed`/`expired`, acontecem na mesma transação PostgreSQL. O worker processa o lote com lease renovável por item, limita transições a um compare-and-set com lease ainda futuro, aplica timeout, backoff exponencial com jitter e máximo de tentativas. Ao exceder o limite, usa a combinação `failed` + `available_at` terminal já existente como dead-letter, sem introduzir estado/migration durante PLAT-004; esse registro fica visível para operação e não é reivindicado novamente.

Rate limits de identidade resolvem o IP por header encaminhado somente quando `CASEI_TRUSTED_PROXIES` contém CIDRs válidos dos proxies reversos e a origem está protegida contra acesso direto. Sem essa configuração, headers de IP enviados pelo cliente são ignorados, evitando spoofing, e o Better Auth usa o bucket compartilhado seguro. `CASEI_CLIENT_IP_HEADERS` permite escolher headers equivalentes somente dentro desse modo confiável.
