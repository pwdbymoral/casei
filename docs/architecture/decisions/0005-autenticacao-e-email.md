...
O Better Auth `runInBackgroundOrAwait` registra e absorve rejeições do callback. Por isso a aplicação registra cada falha de enqueue em um `AuthEmailEnqueueFailureSink` sanitizado, tenta novamente a persistência com limite curto e deixa a mensagem completa disponível somente ao mecanismo controlado de recovery/reenvio; a resposta pública não é usada como confirmação de entrega. A inserção da intent e da outbox, e as transições `queued`/`sent`/`failed`/`expired`, acontecem na mesma transação PostgreSQL. O worker renova o lease durante uma entrega lenta, aplica timeout, backoff exponencial com jitter e máximo de tentativas. Ao exceder o limite, usa a combinação `failed` + `available_at` terminal já existente como dead-letter, sem introduzir estado/migration durante PLAT-004; esse registro fica visível para operação e não é reivindicado novamente.
