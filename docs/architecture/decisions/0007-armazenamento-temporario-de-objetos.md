...
Downloads usam streaming autorizado ou URL assinada de curta duração, criada somente após revalidar sessão, membership e estado do job. Upload direto assinado fica fora da primeira implementação de 10 MB, mas o port permite adicioná-lo.
...
- Presigned URLs precisam de TTL curto e não substituem autorização no momento da emissão.
