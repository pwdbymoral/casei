...
O processo DATA-006 usa `pnpm --filter @casei/api worker:export`, também separado da API, e compartilha `DATABASE_URL_WORKER`, `DATABASE_ROLE` e `CASEI_OBJECT_STORAGE_*` com o bootstrap padrão. A API só registra o adapter PostgreSQL + S3 quando `CASEI_OBJECT_STORAGE_BUCKET` está configurado; em produção, bucket e região são obrigatórios. O download continua passando pela API autorizada, sem URL presignada bearer. Para desenvolvimento, use `pnpm --filter @casei/api worker:export:dev` após configurar o storage compatível com S3.
